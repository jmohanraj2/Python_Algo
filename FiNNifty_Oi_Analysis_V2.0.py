...
expiry = "12-Dec-2023" # DEC 19, 26  Jan - 02, 09, 30 

excel_file = "OptionChain_Analysis_finn.xlsx"
wb = xw.Book(excel_file)
main_sht = wb.sheets('OI_Data')
sht_live = wb.sheets('Data')
#pe_sht = wb.sheets('PE_Data')
#ce_sht = wb.sheets('CE_Data')
Underlying_Man_FN = 21260 #  +/- 700
...
cookies = {
    "bm_sv" : "6B2AB980F922C484578437A17524F65~~1;"
}
...
oi_filename = os.path.join("Files","oi_data_records_FN_{0}.json".format(currdttime))
mp_filename = os.path.join("Files","mp_data_records_FN_{0}.json".format(currdttime))

def fetch_oi(df,mp_df):
    #removed dfce and dfpe in above step
    tries =1
    max_tries =3
    ...
    #Filtering Options Chain
            uplimit = (Underlying_Man_FN + 700)
            lowlimit = (Underlying_Man_FN - 700)
            print("fist loc")
    ...
    #df is the empty dataset in first run and it will be appended to df1 new dataset
           ...
           # print("MP_DF",mp_df['MaxPain'])
            ...
            uplimit = (Underlying_Man_FN + 700)
            lowlimit = (Underlying_Man_FN - 700)
            #df4['uplimit']  = (mp_df['underlying'] + 500).astype('int')
            #df4['lowlimit'] = (mp_df['underlying'] - 500).astype('int')
